...
class LoanAccount: 
    ...
    def calculate_monthly_payment(self): 
        ...
        return self.loan_amount * rate / (1 - (1 + rate) ** -term_months)

    def make_payment(self, amount): 
        if amount >= self.monthly_payment: 
            self.remaining_balance -= amount 
            self.payment_history.append((datetime.datetime.now(), amount)) 
            print(f"Loan payment of {amount} made. Remaining balance: {self.remaining_balance}") 
        else: 
            print(f"Payment is less than the required {self.monthly_payment}") 
...
class Customer: 
    ...
    def apply_for_loan(self, loan_amount, interest_rate, term_years): 
        if loan_amount > 0: 
            self.loan_account = LoanAccount(self.bank_account, loan_amount, interest_rate, term_years) 
            print(f"{self.name} applied for a loan of {loan_amount} at {interest_rate*100}% interest for {term_years} years") 
        else: 
...
class Bank: 
    ...
    def loan_risk_assessment(self): 
        ...
        for loan in self.loans: 
            ...
            if risk_level > 0.75: 
                high_risk_loans.append((loan.account.name, loan.get_balance(), risk_level))
...
class FixedDepositAccount(BankAccount): 
    def __init__(self, name, balance=0, term_years=5, interest_rate=0.04): 
        ...
        self.term_years = term_years 
        self.interest_rate = interest_rate 
        self.maturity_date = datetime.datetime.now() + datetime.timedelta(days=365 * term_years) 
    ...
    def apply_interest(self): 
        if datetime.datetime.now() >= self.maturity_date: 
            ...
            print(f"Account will mature on {self.maturity_date}. Interest cannot be applied yet.") 
